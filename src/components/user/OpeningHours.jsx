...
class OpeningHours extends React.Component {
  state = {
    opening_hours: this.props.opening_hours.opening_hours,
    test: "",
    day: "Monday",
    open_hr: "12",
    open_min: "00",
  ...
  updateHours = async (string_data) => {
    await fetch(
      `${process.env.REACT_APP_BACKEND_URL}/restaurants/${this.props.restaurantId}`,
      {
        method: "PUT",
        headers: {
    ...
    const test = {
      opening_hours: [
        ...this.state.opening_hours,
        {
          day: this.state.day,
          opening_hours: {
            open_hr: this.state.open_hr,
            open_min: this.state.open_min,
            open_ampm: this.state.open_ampm,
          },
          closing_hours: {
            close_hr: this.state.close_hr,
            close_min: this.state.close_min,
            close_ampm: this.state.close_ampm,
          },
        },
      ],
    };
  ...
  saveEditHours = (event) => {
    ...
    opening_hours[this.state.edit_oh_index] = {
      day: this.state.edit_day,
      opening_hours: {
        open_hr: this.state.edit_open_hr,
        open_min: this.state.edit_open_min,
        open_ampm: this.state.edit_open_ampm,
      },
      closing_hours: {
        close_hr: this.state.edit_close_hr,
        close_min: this.state.edit_close_min,
        close_ampm: this.state.edit_close_ampm,
      },
    };
  ...
  editHours = (i, oh) => {
    this.setState({
      edit_oh_index: i,
      edit_day: oh.day,
      edit_open_hr: oh.opening_hours.open_hr,
      edit_open_min: oh.opening_hours.open_min,
      edit_open_ampm: oh.opening_hours.open_ampm,
      edit_close_hr: oh.closing_hours.close_hr,
      edit_close_min: oh.closing_hours.close_min,
      edit_close_ampm: oh.closing_hours.close_ampm,
    });
  };
          ...
          {this.state.opening_hours.map((oh, index) => {
            return index === this.state.edit_oh_index ? (
              ...
              <ul key={index}>
                <li>{oh.day}</li>
                <li>
                  {oh.opening_hours.open_hr}:{oh.opening_hours.open_min}
                  {oh.opening_hours.open_ampm}
                </li>
                <li>
                ...
                <li>
                  <button onClick={() => this.editHours(index, oh)}>
                    Edit
                  </button>
                  <button onClick={() => this.deleteHours(index)}>
                    Delete
                  </button>
                </li>
              </ul>
