...
class Menu extends React.Component {
  ...
  updateMenu = async (new_title, id) => {
    ...
    await fetch(`${process.env.REACT_APP_BACKEND_URL}/restaurants/${this.state.restaurant.id}/menus/${id}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
        Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  deleteMenu = async (rest_id, menu_id, index) => {
    await fetch(`${process.env.REACT_APP_BACKEND_URL}/restaurants/${rest_id}/menus/${menu_id}`, {
      method: "DELETE",
      headers: {
        Authorization: `Bearer ${localStorage.getItem("token")}`,
      },
  ...
  updateState = () => {
    this.setState({
      restaurant: this.context.restaurants.find((restaurant) => restaurant.id === this.state.restaurant.id),
    });
  };
            ...
            <div key={index} className="menu"> 
              <div className={"header"}>
                ...
                <EditableField
                  ...
                  id={menu.id}
                />
                <button onClick={() => this.deleteMenu(menu.restaurant_id, menu.id, index)}>
                  Delete Menu
                </button>
              </div>
